 ...
 */
package org.apache.marmotta.loader.cassandra;
 ...
 *
 * @author David Fernández (luskaner@gmail.com)
 */
public class CassandraLoaderBackend implements LoaderBackend {
    ...
    public LoaderHandler createLoader(Configuration configuration) {
        ...
        titanCfg.setProperty("storage.backend", "cassandra");
        //titanCfg.setProperty("storage.batch-loading", true);

        if(configuration.containsKey("backend.cassandra.host")) {
            titanCfg.setProperty("storage.hostname", configuration.getString("backend.cassandra.host"));
        }
        if(configuration.containsKey("backend.cassandra.port")) {
            titanCfg.setProperty("storage.port", configuration.getInt("backend.cassandra.port"));
        }
        if(configuration.containsKey("backend.cassandra.table")) {
            titanCfg.setProperty("storage.tablename", configuration.getString("backend.cassandra.table"));
        }

        titanCfg.setProperty("ids.block-size", configuration.getInt("backend.cassandra.id-block-size", 500000));
    ...
    public Collection<Option> getOptions() {
        ...
        Option host =
                OptionBuilder.withArgName("host")
                                .hasArgs(1)
                                .withDescription("hostname or IP address of Cassandra server")
                                .withLongOpt("host")
                                .create('H');
        options.add(host);
        ...
        Option port =
                OptionBuilder.withArgName("port")
                        .hasArgs(1)
                        .withDescription("port used by Cassandra server")
                        .withLongOpt("port")
                        .create('P');
        options.add(port);
        ...
        Option table =
                OptionBuilder.withArgName("table")
                        .hasArgs(1)
                        .withDescription("database table used by Cassandra server")
                        .withLongOpt("table")
                        .create('T');
        options.add(table);
